...
export class ModelInfoService {
  ...
  async lookupModelInfo(modelId: string, skipCache: boolean = false): Promise<ModelInfo | null> {
    // For now, return a basic info object to satisfy the route
    return { name: modelId, source: 'huggingface' };
  }

  private async fetchFromHuggingFace(modelId: string): Promise<ModelInfo | null> {
    try {
