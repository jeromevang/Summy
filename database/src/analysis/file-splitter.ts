...
import { ASTParser, SemanticUnit } from './ast-parser.js';
...
export class FileSplitter {
  ...
  async splitFile(filePath: string, outputDir: string, options: SplitOptions = {}): Promise<void> {
  ...
  private generateFileContent(unit: SemanticUnit, originalContent: string): string {
  ...
  private updateIndex(originalPath: string, unit: SemanticUnit, newPath: string): void {
    ...
    connection.prepare(`
      ...
      ON CONFLICT(id) DO UPDATE SET
        ...
        updated_at = CURRENT_TIMESTAMP
    `).run(
      `${unit.name}_${Date.now()}`,
      newPath,
      unit.name,
      unit.type,
      unit.content,
      1,
      unit.endLine - unit.startLine + 1
    );
  }
}
